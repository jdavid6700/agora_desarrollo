...
class FormularioObjeto {
	...
	function formulario() {
		...
		// ---------------- FIN CONTROL: Inicio Marco --------------------------------------------------------
				...
				// ---------------- CONTROL: Cuadro de Texto - OBJETO CONTRATO --------------------------------------------------------
			...
			echo $this->miFormulario->marcoAgrupacion ( 'inicio', $atributos );
				...
				// ---------------- CONTROL: Lista grupo CIIU--------------------------------------------------------
					...
					$atributos ['anchoCaja'] = 60;
					$atributos ['miEvento'] = '';
					$atributos ['cadena_sql'] = $cadenaSql = $this->miSql->getCadenaSql ( 'ciiuGrupo', '1' );
					$matrizItems = $esteRecursoDB->ejecutarAcceso ( $cadenaSql, "busqueda" );
					$atributos ['matrizItems'] = $matrizItems;
					$atributos = array_merge ( $atributos, $atributosGlobales );
					echo $this->miFormulario->campoCuadroLista ( $atributos );
				...
				// ---------------- CONTROL: Lista clase CIIU--------------------------------------------------------
					...
					$atributos ['anchoCaja'] = 60;
					$atributos ['miEvento'] = '';
					$atributos ['cadena_sql'] = $cadenaSql = $this->miSql->getCadenaSql ( 'ciiuClase', '11' );
					$matrizItems = $esteRecursoDB->ejecutarAcceso ( $cadenaSql, "busqueda" );
					$atributos ['matrizItems'] = $matrizItems;
					$atributos = array_merge ( $atributos, $atributosGlobales );
					echo $this->miFormulario->campoCuadroLista ( $atributos );
...
// ---------------- CONTROL: Cuadro Lista --------------------------------------------------------
    ...
    $atributos ['cadena_sql'] = $this->miSql->getCadenaSql ( "buscarDependencia" );
...
$miSeleccionador = new FormularioObjeto ( $this->lenguaje, $this->miFormulario, $this->sql );
