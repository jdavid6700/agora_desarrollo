...
class registrarForm {
	...
	function miForm() {
        ...
        $codigoCIIU = $objetoEspecifico[0][1]; 
		$codigoCIIU = '0112';//SE DEBE BORRAR ESTA LINEA
		...
		echo "<span class='textoElegante textoEnorme textoAzul'>Actividad econ&oacute;mica : </span>"; 
                echo "<span class='textoElegante textoMediano textoGris'>". $codigoCIIU . "</span></br>"; 
		...
		$cadenaSql = $this->miSql->getCadenaSql ( 'verificarActividad', $codigoCIIU);
		...
		if( $resultado ){
			...
			//------- FALTA FILTRAR POR ACTIVIDAD ECONOMICA
			//------- FALTA FILTRAR LAS MEJORES CALIFICACIONES MAYORES A 45
			$cadenaSql = $this->miSql->getCadenaSql ( 'proveedoresByClasificacion', $datos );
			$resultadoCalificacion = $esteRecursoDB->ejecutarAcceso ( $cadenaSql, "busqueda" );
			...
			echo $this->miFormulario->marcoAgrupacion ( 'inicio', $atributos );		
				...
				<?php 
				foreach ($resultadoCalificacion as $dato):
					...
					//cargo campo de ID proveedore
					$esteCampo = 'idProveedor' . $dato['id_proveedor'];
					...
					$atributos ['marco'] = true;
					$atributos ["etiqueta"] = "";
					$atributos ['valor'] = $dato['id_proveedor'] ;
		...
		//MENSAJE DE ERROR CUANDO NO HAY PROVEEDORES CON LA ACTIVIDAD ECONOMICA
		}else{
