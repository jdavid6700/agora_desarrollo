...
/*            if (isset($_REQUEST ['id_contrato']) && $_REQUEST ['id_contrato'] != '') {
            ...
            */
            ...
            $cadenaSql = $this->miSql->getCadenaSql('obtenerInfoAuditoriaAgoraAll', $arreglo);
        ...
        $atributos ['tipoEtiqueta'] = 'inicio';
        $atributos ["leyenda"] = "Log Eventos RCL COTIZACIÓN";
        echo $this->miFormulario->marcoAgrupacion('inicio', $atributos);


        if ($contratos) {

            echo "<table id='tablaContratosAprobadosError'>";
            ...
            foreach ($contratos as $valor) {
                ...
                $variable .= "&opcion=consultarContratoAuditError";
                $variable .= "&id=" . $valor ['id'];
                $variable .= "&tiempo=" . $_REQUEST['tiempo'];
                $variable = $this->miConfigurador->fabricaConexiones->crypto->codificar_url($variable, $directorio);

                $mostrarHtml = "<tr><td><center>RC-". date("Y", strtotime($valor ['fecha_log'])) ."-". $valor['id'] . "</td>";
                $mostrarHtml .= "
                    <td><center>" . $valor ['numero_cotizacion'] . "</center></td>";
                $mostrarHtml .= "<td><center>" . $valor['modulo'] . "</td>";
