<?php
use gestionParametros\gestionarSupervisor\Sql;
...
if($secure){
	...
	if ($_REQUEST ['funcion'] == 'consultarDependencia') {
		if($_REQUEST['valor']==2){
			$cadenaSql = $this->sql->getCadenaSql ( 'dependenciaUdistritalOrdenador', $_REQUEST['valor'] );
		    $resultado = $esteRecursoDB->ejecutarAcceso ( $cadenaSql, "busqueda" );
		}else{
			$cadenaSql = $this->sql->getCadenaSql ( 'dependenciaUdistritalSupervisor', $_REQUEST['valor'] );
			$resultado = $esteRecursoDB->ejecutarAcceso ( $cadenaSql, "busqueda" );
		}
