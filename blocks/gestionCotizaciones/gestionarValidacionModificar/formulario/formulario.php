...
class Formulario {
	...
	function formulario() {
		...
		$this->cadena_sql = $this->miSql->getCadenaSql ( "ordenadorUdistritalDoc" );
		$listaOrdenadores = $esteRecursoDB->ejecutarAcceso ( $this->cadena_sql, "busqueda" );
		
		$this->cadena_sql = $this->miSql->getCadenaSql ( "dependenciaUdistritalDoc" );
		$listaJefesDepen = $esteRecursoDB->ejecutarAcceso ( $this->cadena_sql, "busqueda" );
		
		
		$this->cadena_sql = $this->miSql->getCadenaSql ( "ordenadorUdistritalByDoc", $resultadoUsuario[0]['identificacion'] );
		$confirmarOrdenador = $esteRecursoDB->ejecutarAcceso ( $this->cadena_sql, "busqueda" );
		

		$this->cadena_sql = $this->miSql->getCadenaSql ( "listarObjetosParaModificacionByOrdenador", $confirmarOrdenador[0]['id'] );
		$resultado = $esteRecursoDB->ejecutarAcceso ( $this->cadena_sql, "busqueda" );
		...
		if ($resultado) {
			...
			$tipo = 'success';
			$mensaje =  "Lista de Solicitudes de Cotización con Peticiones de Modificación. </b>
				</br>
				...
				</br><b>Ordenador del Gasto: </b>".$confirmarOrdenador[0]['ordenador']."		
						";
		...
		echo $this->miFormulario->marcoAgrupacion ( 'inicio', $atributos );
			...
			echo "<thead>
							<tr>
								<th><center>Solicitud Modificación</center></th>
								<th><center>Número Cotización</center></th>
			...
			$this->cadena_sql = $this->miSql->getCadenaSql ( "estadoPeticionModificacion", $dato['pk'] );
			$resultadoEstPet = $esteRecursoDB->ejecutarAcceso ( $this->cadena_sql, "busqueda" );
			...
			if(isset($dato['estado_cotizacion'])){//CAST
				...
				if($resultadoEstPet[0]['estado_solicitud'] == 1){
					
					$variableCan = "pagina=" . $miPaginaActual; // pendiente la pagina para modificar parametro
					$variableCan .= "&opcion=aprobarSolicitud";
					$variableCan .= "&idSolicitudModificacion=" . $dato['pk'];
			...
			$mostrarHtml = "<tr>
									<td><center>" . "SMC-" . sprintf("%06d", $dato['pk']) . "</center></td>
								   ...
								   "<td><center>" . $dato['tipo_necesidad'] . "</center></td>
									...
									<td><center>" . $resultadoEstPet[0]['nombre'] . "</center></td>				
...
<?php
			...
			$atributos ["mensaje"] = "Actualmente no hay Solicitudes de Cotización Disponibles con Peticiones de Modificación para 
