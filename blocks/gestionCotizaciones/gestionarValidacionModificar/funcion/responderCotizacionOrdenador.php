...
class SolicitudCotizacion {
	...
	function procesarFormulario() {
		...
		if(isset($_REQUEST['decision']) && $_REQUEST['decision'] == "1"){// Cotización APROBADA por el Ordenador
			...
			$datos = array (
					'solicitud_modificacion_cotizacion' => $_REQUEST['idObjeto'],
			...
			$validacionOrdenador = $this->miSql->getCadenaSql ( 'validacionOrdenadorMod', $datos );
		...
		$actualizoModCotizacion = $esteRecursoDB->transaccion($SQLs);
		
		
		if ($actualizoModCotizacion) {
