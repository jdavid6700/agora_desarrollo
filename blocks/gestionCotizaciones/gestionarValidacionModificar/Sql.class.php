    ...
    function getCadenaSql($tipo, $variable = "") {
        ...
        switch ($tipo) {
        	...
        	case "validacionOrdenadorMod" :
        		...
        		$cadenaSql .= " '" . $variable ['solicitud_modificacion_cotizacion'] . "'";
        		$cadenaSql .= " )";
        		$cadenaSql .= " ;";
        		break;
        	
        	case "infoPeticionModificacion" :
        		$cadenaSql=" SELECT RESM.id, RESM.responsable, RESM.justificacion, RESM.fecha_estado, RESM.estado_solicitud, ESM.nombre";
        		...
        		$cadenaSql.=" WHERE RESM.solicitud_modificacion_cotizacion = '" . $variable . "' AND RESM.estado_solicitud = 1";
        		$cadenaSql.=" ORDER BY RESM.fecha_estado DESC LIMIT 1";
        		break;
        	
        	case "estadoPeticionModificacion" :
        		$cadenaSql=" SELECT RESM.id, RESM.responsable, RESM.fecha_estado, RESM.estado_solicitud, ESM.nombre";
        		$cadenaSql.=" FROM agora.relacion_estado_solicitud_modificacion RESM";
        		$cadenaSql.=" JOIN agora.estado_solicitud_modificacion ESM ON ESM.id = RESM.estado_solicitud";
        	...
        	case "listarObjetosParaModificacionByOrdenador" :
        		$cadenaSql = " SELECT ";
        		$cadenaSql.=" SMC.id as pk, OC.* ";
        		$cadenaSql.=" FROM agora.solicitud_modificacion_cotizacion SMC ";
        		$cadenaSql.=" JOIN agora.objeto_cotizacion OC ON OC.id = SMC.objeto_cotizacion";
        		$cadenaSql.=" WHERE OC.ordenador_gasto = '" . $variable . "';";
        		break;
            ...
            case "listarObjetosParaCotizacionByOrdenador" :
                ...
                $cadenaSql.=" WHERE ordenador_gasto = '" . $variable . "' AND estado_cotizacion = 7;";
                break;

            case "ordenadorUdistritalByDoc" :
                $cadenaSql = " SELECT DISTINCT ON (CR.id) CR.id , ('(' ||JD.tercero_id || ') - ' || UPPER(CR.cargo) ) as ordenador , DP.nombre, JD.fecha_inicio, JD.fecha_fin, JD.tercero_id";
                $cadenaSql.=" FROM core.ordenador_gasto CR";
                $cadenaSql.=" JOIN oikos.dependencia DP ON CR.dependencia_id = DP.id";
                $cadenaSql.=" JOIN core.jefe_dependencia JD ON JD.dependencia_id = DP.id";
                $cadenaSql.=" WHERE JD.tercero_id = " . $variable;
                $cadenaSql.=" GROUP BY CR.id, CR.cargo, DP.nombre, JD.tercero_id, JD.fecha_inicio, JD.fecha_fin";
                $cadenaSql.=" ORDER BY CR.id, CR.cargo, JD.fecha_fin DESC";

                break;

            case "ordenadorUdistritalDoc" :
                $cadenaSql = " SELECT DISTINCT ON (CR.id) CR.id , ('(' ||JD.tercero_id || ') - ' || UPPER(CR.cargo) ) as ordenador , DP.nombre, JD.fecha_inicio, JD.fecha_fin, JD.tercero_id";
                $cadenaSql.=" FROM core.ordenador_gasto CR";
                $cadenaSql.=" JOIN oikos.dependencia DP ON CR.dependencia_id = DP.id";
                $cadenaSql.=" JOIN core.jefe_dependencia JD ON JD.dependencia_id = DP.id";
